...
class FileTransferClient:
    ...
    async def upload_file(
        ...
        try:
            ...
            if options.verify_checksum:
                ...
                self.logger.debug(f"Local {options.checksum_type.value}: {local_checksum.value}")
    ...
    async def download_file(
        ...
        try:
            ...
            if options.verify_checksum:
                ...
                self.logger.debug(f"Remote {options.checksum_type.value}: {remote_checksum}")
    ...
    async def _setup_data_channel(self, session_data: dict) -> Any:
        ...
        websocket_config = create_websocket_config(
            ...
            token=session_data["token_value"]
        )
    ...
    async def _upload_file_data(
        ...
        try:
            if options.encoding == FileTransferEncoding.BASE64:
                return await self._upload_base64(data_channel, local_file, remote_path, options)
            elif options.encoding == FileTransferEncoding.RAW:
                return await self._upload_raw(data_channel, local_file, remote_path, options)  
            elif options.encoding == FileTransferEncoding.UUENCODE:
                return await self._upload_uuencode(data_channel, local_file, remote_path, options)
            else:
    ...
    async def _upload_raw(
        ...
        raise NotImplementedError("Raw binary upload requires terminal binary mode support")
    ...
    async def _download_file_data(
        ...
        if options.encoding == FileTransferEncoding.BASE64:
            return await self._download_base64(data_channel, remote_path, local_file, options)
        else:
            raise NotImplementedError(f"Download with {options.encoding} not yet implemented")
    ...
    async def _get_remote_checksum(
        ...
        await data_channel.send_input_data(cmd.encode())
        
        # Wait for command completion
        await asyncio.sleep(1)
        
        # Parse checksum from output
        all_data = b''.join(data_channel._received_data)  # type: ignore
        output = all_data.decode('utf-8', errors='ignore')
        
        # Look for checksum pattern (hex string followed by filename)
        lines = output.split('\n')
        for line in lines:
            line = line.strip()
            if line and len(line) >= 32:  # At least MD5 length
                parts = line.split()
                if len(parts) >= 2:
        ...
        raise RuntimeError(f"Could not parse {checksum_type.value} checksum from: {output}")