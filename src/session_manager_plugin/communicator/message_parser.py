...
class MessageParser:
    ...
    def parse_websocket_message(self, message: WebSocketMessage) -> Optional[ParsedMessage]:
    ...
    def _parse_binary_message(self, message: WebSocketMessage) -> Optional[ParsedMessage]:
        ...
        if client_message.payload_type == PayloadType.HANDSHAKE_REQUEST:
            ...
            self.logger.debug(f"Unknown binary message type: {client_message.message_type}")
            return ParsedMessage(
                message_type=ParsedMessageType.UNKNOWN_BINARY,
                client_message=client_message,
                raw_data=message.data
    ...
    def _parse_text_message(self, message: WebSocketMessage) -> Optional[ParsedMessage]:
        ...
        return ParsedMessage(
            message_type=ParsedMessageType.TEXT_CONTROL,
    ...
    def _parse_json_payload(self, payload: bytes) -> Dict[str, Any]:
        ...
        try:
            result: Dict[str, Any] = json.loads(payload.decode("utf-8", errors="ignore"))
            return result
        except Exception as e:
